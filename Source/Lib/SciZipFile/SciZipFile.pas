...
type
  ...
  private
    ...
    function GetAttr(i: integer): Cardinal;
  public
    ...
    property Attr[i: integer]: cardinal read GetAttr;
  end;
...
begin
    ...
    begin
      ...
      SizeOfTheCentralDirectory :=
        ZipFileStream.Position - Integer(OffsetOfStartOfCentralDirectory);
...
begin
  ...
  // Pepak - support for stored files
  if Files[i].CommonFileHeader.CompressionMethod = 0 then
    begin
    ...
    try {+}
      ...
      try {+}
        ...
        if ReadBytes <> integer(Files[i].CommonFileHeader.UncompressedSize) then
          Raise EZipFileCRCError.Create('Invalid size'); //Result := '';
      finally
        Decompressor.Free;
        end;
    finally
...
function TZipFile.GetAttr(i: integer): cardinal;
begin
  if i > High(Files) then // begin Result:=0; exit; end;
    raise Exception.Create('Index out of range.');
  Result := CentralDirectory[i].ExternalFileAttributes;
end;
