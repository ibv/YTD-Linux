...
{$ENDIF}
  ...
  public
    constructor Create; virtual;
    destructor Destroy; override;
    function AddNil: Integer; overload;
...
function TryStrToDateTime(const str: string; out dt: TDateTime): boolean;
begin
  try
...
var
  stream: TFileStream;
begin
  stream := TFileStream.Create(FileName, fmOpenRead {$ifdef LEGACYVERSION}or{$else},{$endif} fmShareDenyWrite);
  try
...
function TSuperArray.AddNil: Integer;
begin
  Result := FLength;
  PutO(Result, nil);
end;
